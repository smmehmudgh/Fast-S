<div align="center">

# ⚡ Fast S

### A lightning-fast, real-time chat experience — built for the modern web.

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&duration=3000&pause=800&color=00D4FF&center=true&vCenter=true&width=700&lines=Real-time+messaging+%E2%9A%A1;Beautiful+UI+%F0%9F%8E%A8;Group+chats+%F0%9F%91%A5;Lightning+fast+%F0%9F%9A%80" alt="Typing SVG" />

<br>

[![Made with ❤️](https://img.shields.io/badge/Made%20with-%E2%9D%A4%EF%B8%8F-ff2e93?style=for-the-badge)](https://github.com/your-username/fast-s)
[![Real-time](https://img.shields.io/badge/Real--time-Firestore-00d4ff?style=for-the-badge)](https://firebase.google.com)
[![Open Source](https://img.shields.io/badge/Open%20Source-%E2%9C%93-7b2ff7?style=for-the-badge)](https://github.com/your-username/fast-s)

<br>

[![Typing](https://img.shields.io/badge/Typing%20Indicator-Live-00d4ff?style=flat-square)](#-features)
[![Groups](https://img.shields.io/badge/Group%20Chats-Unlimited-7b2ff7?style=flat-square)](#-features)
[![Invites](https://img.shields.io/badge/Group%20Invites-✓-00e676?style=flat-square)](#-features)
[![Reactions](https://img.shields.io/badge/Reactions-6%20Emojis-ff2e93?style=flat-square)](#-features)
[![Dark Mode](https://img.shields.io/badge/Dark%20Mode-%E2%9C%93-ffc107?style=flat-square)](#-features)

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%" height="3px">

</div>

<br>

## 🌟 What is Fast S?

**Fast S** is a beautifully crafted, real-time chat application that brings people together instantly. From quick one-on-one messages to full group conversations with an invite-based join flow — Fast S makes every chat feel alive.

> 💡 **Simple. Fast. Beautiful.** No clutter, no lag, just pure conversation.

<br>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%" height="3px">

## ✨ Features

<br>

### 💬 **Real-Time Messaging**

<table>
<tr>
<td width="50%">

- ⚡ **Instant delivery** — messages appear the moment they're sent
- ✓✓ **Read receipts** — know exactly when your message was seen
- ✍️ **Typing indicator** — watch the dots dance while someone types
- 🟢 **Online status** — see who's available right now
- 🕐 **Smart timestamps** — "just now", "2m ago", "yesterday"

</td>
<td width="50%">

- ↩️ **Reply** to any message
- ✏️ **Edit** your sent messages
- 🗑️ **Delete for me** or **Delete for everyone**
- ↪️ **Forward** to any chat instantly
- ⭐ **Star** messages you want to keep

</td>
</tr>
</table>

<br>

### 😍 **Expressive Reactions**

React to any message with a single tap — no emoji picker needed.

<div align="center">

| ❤️ | 😂 | 👍 | 😮 | 😢 | 🙏 |
|:---:|:---:|:---:|:---:|:---:|:---:|
| Love | Laugh | Like | Wow | Sad | Thanks |

</div>

<br>

### 👥 **Friends & Social**

- 🔍 **Smart search** — find people by name or `@username`
- ➕ **Friend requests** — send, accept, or politely decline
- 🔔 **Instant notifications** — never miss a request
- 🚫 **Block & report** — full control over your space
- ❌ **Unfriend** with a safety confirmation

<br>

### 👨‍👩‍👧‍👦 **Group Chats** (Invite-Based)

Create groups with **unlimited members** — no cap, no limit.

<div align="center">

| 🎉 Create | 📩 Invite | 👑 Manage |
|:---:|:---:|:---:|
| Search any user, add them from friends or by username | Invitees get a preview & join on their terms | Admins can kick, promote, edit, and add more |

</div>

- 🔒 **Invite-only** — no one is added without consent
- 👀 **Preview before joining** — see the group, admin, and members
- ✅ **Join / Decline** — accept on your own terms
- 👑 **Admin controls** — promote members, kick, edit name/bio/avatar
- 📊 **Live member list** with admin badges
- 🖼️ **Custom group avatars** with compression

<br>

### 📎 **Rich Media Sharing**

<div align="center">

| 🖼️ Images | 📄 Files |
|:---:|:---:|
| Auto-compressed, sharp & fast | Up to 500KB, instantly delivered |

</div>

Share photos and documents without ever leaving the chat.

<br>

### 🎨 **Beautiful Experience**

<table>
<tr>
<td width="50%">

- 🌓 **Dark / Light mode** — your eyes, your choice
- 📱 **Fully responsive** — mobile, tablet, desktop
- ✨ **Smooth animations** — every tap feels alive
- 🎭 **Loading states** — perceived speed matters

</td>
<td width="50%">

- 🔙 **Smart back button** — no jarring jumps
- 🔔 **Notification bell** — unread count at a glance
- 🎯 **Context menus** — right-click or long-press
- 💬 **Friendly toasts** — every action confirmed

</td>
</tr>
</table>

<br>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%" height="3px">

## 🎯 Why Fast S?

<div align="center">

| 🚀 | 🎨 | 🔒 | 💚 |
|:---:|:---:|:---:|:---:|
| **Fast** | **Beautiful** | **Secure** | **Free** |
| Instant delivery | Neon-inspired UI | Encrypted auth | No paywall |

</div>

<br>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%" height="3px">

## 📸 Screenshots

<div align="center">

### 💬 Your Conversations

<img src="https://via.placeholder.com/800x450/0a0e27/00d4ff?text=Chat+List" alt="Chats" width="90%">

<br><br>

### 💭 Real-Time Messaging

<img src="https://via.placeholder.com/800x450/0a0e27/7b2ff7?text=Chat+View" alt="Chat" width="90%">

<br><br>

### 👥 Group Info & Members

<img src="https://via.placeholder.com/800x450/0a0e27/00e676?text=Group+Info" alt="Group Info" width="90%">

<br><br>

### 👤 Your Profile

<img src="https://via.placeholder.com/800x450/0a0e27/ff2e93?text=Profile" alt="Profile" width="90%">

</div>

<br>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%" height="3px">

## 🚀 Coming Soon

<div align="center">

- 🎙️ Voice messages
- 📞 Voice & video calls
- 📌 Pin, mute & archive chats
- 🔍 In-chat message search
- 📱 Install as an app (PWA)
- 🌍 Multi-language support
- 🤖 AI-assisted replies

</div>

<br>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%" height="3px">

## 👨‍💻 Meet the Developer

<div align="center">

**Sazid Mehmud**

*Building the future, one line at a time.*

<br>

[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/your-username)
[![Twitter](https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white)](https://twitter.com/your-handle)

</div>

<br>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%" height="3px">

## ⭐ Support the Project

<div align="center">

**If Fast S brings joy to your conversations,**

**please consider giving it a star!**

<br>

[![Star this repo](https://img.shields.io/badge/⭐%20Star%20this%20repo-ffc107?style=for-the-badge)](https://github.com/your-username/fast-s/stargazers)

<br>

<img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%" height="3px">

### Made with ❤️ and lots of ☕

**© 2026 Fast S — Chat, reimagined.**

</div>
